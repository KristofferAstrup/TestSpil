...
import static State.IState.State.Game;
...
public class DebugController {
    ...
    private DebugCommand[] debugCommands = new DebugCommand[]{
            ...
            new DebugCommand("delete (?<objgroupname>[A-Za-z]*) (?<xpos>[0-9]*) (?<ypos>[0-9]*)", (matcher) -> {
                ...
                return true;
            },"delete [objgroupname] [xpos] [ypos]","Creates a given WorldObject at a given position"),

            new DebugCommand("delete (?<debugname>[A-Za-z]*)", (matcher) -> {
                if(!debugWorldObjects.containsKey(matcher.group("debugname"))){return false;}
                //EditorState.deleteObj(debugWorldObjects.get(matcher.group("debugname")));
                return true;
            },"delete [objgroupname] [xpos] [ypos]","Creates a given WorldObject at a given position"),
            ...
            new DebugCommand("teleport (?<debugname>[A-Za-z0-9]*) (?<xpos>[0-9]*) (?<ypos>[0-9]*)", (matcher) -> {
                ...
                return true;
            },"teleport [debugname] [xpos] [ypos]","Sets the position of the WorldObject associated with the given debugname"),
